            ...
            extends Thread
    {
        ...
        public void run()
        {
            try {
                ...
                fc.close();
                log.info(String.format("Done reading %d bytes from %s (using local file %s) at offset %d and length %d for client %s", nread, remotePath, filename, offset, readLength, localDataTransferClient.getRemoteAddress()));
            }
            catch (Exception e) {
                try {
